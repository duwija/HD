...
<section class="content-header">
  ...
  <div class="card card-primary card-outline">
    <div class="card-header">
      ...
      <button type="button" class="float-right btn  bg-gradient-primary btn-sm btn btn-sm btn-primary" data-toggle="modal" data-target="#modal-additeminvoice">     Add Invoice Item</button>
    </div>
  ...
  <tbody>
     ...
     <th style="width: 25%" class="text-right">CID / Name :</th>
      <td><a href="customer/{{ $customer->name}}"><strong>{{$customer->customer_id}} ( {{$customer->name}} )</strong></td>
...
<!-- /.card -->
    ...
    <div class="card-body">
      ...
      @csrf
       <input type="text" name="id_customer" id="id_customer" value={{$customer->id}}>
       @php 
       $subtotal=0;
       $tempcode=0;
       @endphp
         ...
         @php $subtotal = $subtotal + $invoice->amount @endphp
         <tr>
          ...
          @if($invoice->payment_status == 0)
            ...
            <td style="background-color: blue" >{{ 'Unpaid' }}</td>
       ...
       @endphp
        ...
        @endforeach
                <tr> <td colspan="3"> <strong> Sub Total</strong></td>
          <td>
       ...
       $total = $subtotal + $tax;
        ...
        @endphp
             ...
             <tr> <td colspan="3"> <strong> Tax Ppn ({{$taxfee}}%)</strong>
              {{-- <input type="text" name="subtotal" id="subtotal" value={{$subtotal}} >--}}
              <input type="text" name="tax" id="tax" value={{$taxfee}} ></td> 
