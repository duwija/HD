...
<section class="content-header">
  ...
  <div class="card card-outline">
    <div class="card-header bg-primary ">
      ...
      <br>
      @if (empty($date_from))
      <form role="form" method="post" action="/suminvoice/transaction">
        @csrf
     ...
     <form role="form" method="post" action="/suminvoice/transaction">
      ...
      <div class="row float-right  pt-2 col-md-12 m-auto pl-4">
        ...
        <div class="input-group p-1 col-md-2 " >
             {{--   <input type="text" name="updated_by"  class="form-control " placeholder="Receive By" /> --}}
               <select name="updated_by" id="updated_by" class="form-control">
           <option value="">ALL</option>
            @foreach ($user as $id => $name)
            <option value="{{ $id }}">{{ $name }}</option>
    ...
    <tbody>
     ...
     { $badge_sts = "badge-success";
      ...
      @endphp
             ...
             @if ($suminvoice->verify == 1)
     <td class="text-center"><a class="badge text-white badge-primary">VERIFIED </a></td>
                @elseif ($suminvoice->verify == 0 )
                <td class="text-center">
                <form  action="/suminvoice/verify/{{ $suminvoice->id }}" method="POST" class="d-inline invoice-verify" >
                @method('post')
                @csrf

                <a title="Verify" type="submit"  class="badge text-white badge-warning btnblink"> UNVERIFIED  </a>
